...
A tela de acompanhamento consulta a sessão antes de mostrar o formulário. Enquanto aguarda pagamento ou confirmação por e-mail, não pede o endereço novamente nem oferece troca de e-mail. O reenvio reutiliza o e-mail do pedido e o mesmo desafio de autenticação, com limite de envios. O endereço fica vinculado ao pedido desde a criação do checkout; os endpoints de login e checkout rejeitam a tentativa de substituí-lo durante esse acompanhamento. “Corrigir e-mail” aparece somente quando não existe sessão ou acompanhamento válido, para corrigir uma tentativa de login, e não altera pedidos ou compras. Após o prazo da sessão (30 dias por padrão), um novo link autentica a conta existente; as compras e titularidades permanecem no banco.

A página HTML de confirmação usa `Referrer-Policy: strict-origin`: o formulário nativo mantém a origem HTTPS para a validação, sem enviar o token no cabeçalho `Referer`. A validação continua recusando origens externas ou `null`, e mensagens de erro são renderizadas na página de confirmação com ação de retorno.
