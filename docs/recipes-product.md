...
Para uma demonstração local, configure as quatro variáveis em `.env.local` (arquivo ignorado) ou no ambiente do processo, execute `npm run build` e depois `npm run dev:product` em `http://127.0.0.1:4325`. O servidor local serve o build de `dist/` e as mesmas funções sem publicar nada. Não use segredos em `PUBLIC_*`, `VITE_*`, links, HTML ou repositório. `npm run dev` continua servindo a parte institucional estática e não implementa as Functions; para o fluxo completo use o servidor local do produto ou uma implantação de preview na Vercel com configuração mock. Para encerrar a demonstração, defina `RECIPES_PRODUCT_ACCESS_MODE=locked` e `RECIPES_PRODUCT_MOCK_CHECKOUT_ENABLED=false`, depois faça nova implantação/atualização de ambiente. A validação também nega sessões mock anteriores quando o modo muda para `locked`.
